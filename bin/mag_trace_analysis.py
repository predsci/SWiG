...
#          ofm.h5, slogq_r0.h5 (for plotting/analysis) 
#
########################################################################
...
def run(args):
  ...
  expfac_r1_r0 = slice(t_expfac_rss_r0, p_expfac_rss_r0, expfac_rss_r0, r1_rss_t, r1_rss_p)
  ps.wrhdf_2d('expfac_rss_at_r1.h5', p_r1_rss, t_r1_rss, np.transpose(expfac_r1_r0))

  # Get DCHB at rss:
  os.system(bindir+'/ch_distance.py -v -t pfss/rss_r0_t.h5 -p pfss/rss_r0_p.h5 -force_ch -chfile pfss/ofm.h5 -dfile pfss/dchb_rss.h5')
  t_dchb_rss,      p_dchb_rss,      dchb_rss     = ps.rdhdf_2d('pfss/dchb_rss.h5')

  # Get DCHB at r1 through interpolation:  
  dchb_r1 = slice(t_dchb_rss, p_dchb_rss, dchb_rss, r1_rss_t, r1_rss_p) 
  ...
  br_r1_unsigned      = slice(t_br_r1_cs, p_br_r1_cs, br_r1_cs, mesh_r1_trace_t, mesh_r1_trace_p) 

  # Get br_rss mapped to r1:
  br_rss_mapped_to_r1 = slice(t_br_rss_pm_cs, p_br_rss_pm_cs, br_rss_pm_cs, r1_rss_t, r1_rss_p)
...
def slice(t_ch,p_ch,ch_f,t,p):
  interp = RegularGridInterpolator((p_ch, t_ch), ch_f,bounds_error=False,fill_value=0)
  coords=np.vstack((p, t)).reshape(-1,p.shape[1]*p.shape[0])
  values=(interp((coords.T))).reshape(p.shape)
  return values

def remred(xvec,data):
